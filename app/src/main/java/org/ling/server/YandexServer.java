...
import odyssey.ru.linguateacher.Cache;
...
//                                    String w  = java.net.URLEncoder.encode(word, "UTF-8");
                                        String responseString = postTranslate(url3, word);
                                        ...
                                        if (texts.length() == 0) {
                                            //нет данных
                                            handlerTrans.onFailAction("Нет данных", null);
                                            return;
                                        }
                                        if (Integer.valueOf(sobj.getString("code")) > 300) {
                                            //ошибка
                                            handlerTrans.onFailAction("Ошибка", null);
                                            return;
                                        }
                                        ...
                                        for (int i = 0; i < texts.length(); i++) {
                                            ...
                                            if (word.equals(tword)) {
                                                ...
                                                handlerTrans.onFailAction("ошибка слово не переведено", null);
                                                return;
                                            }
